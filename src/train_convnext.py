...
os.environ['CUDA_VISIBLE_DEVICES'] = '1'  # PRIDAJ TOTO - použi GPU:1
...
print(f"Dostupné GPU: {gpus}")
if gpus:
    try:
        tf.config.experimental.set_memory_growth(gpus[0], True)
        print(f"[INFO] Memory growth enabled")
...
from srgan_convxnet import SRGAN_g, SRGAN_d
from config_convxnet import config
...
save_dir = "CONVXNET_MODELS/samples_convxnet_v2"
tlx.files.exists_or_mkdir(save_dir)
checkpoint_dir = "CONVXNET_MODELS/models_convxnet_v2"
tlx.files.exists_or_mkdir(checkpoint_dir)

hr_transform = Compose([
    RandomCrop(size=(384, 384)), # 384 384
    RandomFlipHorizontal(),
])
nor = Compose([Normalize(mean=(127.5), std=(127.5), data_format='HWC'),
              HWC2CHW()])
lr_transform = Resize(size=(96, 96)) # 96 96

train_hr_imgs = tlx.vision.load_images(path=config.TRAIN.hr_img_path, n_threads=32)

# Debug
print(f"Celkový počet: {len(train_hr_imgs)}")
print(f"Typ prvého obrázka: {type(train_hr_imgs[0])}")
none_count = sum(1 for img in train_hr_imgs if img is None)
print(f"Počet None obrázkov: {none_count}")

# Filter
train_hr_imgs = [img for img in train_hr_imgs if img is not None]
print(f"Po filtrovaní: {len(train_hr_imgs)}")
...
class TrainData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # PRIDAJ TOTO - zaisti správny dtype pred akoukoľvek transformáciou
        img = np.asarray(img, dtype=np.uint8)

        hr_patch = self.hr_trans(img)
        hr_patch = np.asarray(hr_patch, dtype=np.uint8)

        lr_patch = self.lr_trans(hr_patch)
        lr_patch = np.asarray(lr_patch, dtype=np.uint8)
...
class WithLoss_G(Module):
    ...
    def forward(self, lr, hr):
        ...
        # BEZ transpozície - VGG je channels_first
...
G = SRGAN_g()
D = SRGAN_d()
VGG = vgg.VGG19(pretrained=True, end_with='pool3', mode='dynamic') # pool4 povodne
...
G.init_build(tlx.nn.Input(shape=(8, 3, 96, 96))) # 8 shape prve
D.init_build(tlx.nn.Input(shape=(8, 3, 384, 384))) # 8 shape prve
...
def evaluate():
    ...
    G.set_eval()
    imid = 100  # 0: 企鹅  81: 蝴蝶 53: 鸟  64: 古堡
